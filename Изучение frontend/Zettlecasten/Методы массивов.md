...
Можно не передавать initial, при его отсутствии в качестве первого значения берется первый элемент массива. Если массив пуст, то вызов reduce без начального значения вызовет ошибку.Поэтому рекомендуется всегда указывать начальное значение.
...
## Array.isArray
Массивы не образуют отдельный тип данных, так как основаны на объектах.
Поэтому typeof не может отличить простой объект от массива. Для массивов есть специальный метод Array.isArray(value), который возвращает true, если value это массив, и false, если нет
