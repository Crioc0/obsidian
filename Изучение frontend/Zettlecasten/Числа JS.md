...
- Используйте округления `Math.floor`(в меньшую), `Math.ceil`(в большую), `Math.trunc`(удаление дробной части, `Math.round`(до ближайшего целого) или `num.toFixed(precision)`(округляет число до n знаков после запятой.
