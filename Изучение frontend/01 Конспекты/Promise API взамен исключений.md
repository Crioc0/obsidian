...
## Что это? (Определение)
API Promise в JS удобно использовать в качестве замены исключений, но Promise имеет асинхронный дизайн, каждый then, catch и finally всегда выполняется отложено (создают микротаску), и если использовать Promise для синхронного кода, будут проблемы с производительностью
